function [s, cx] = mcorrect1(s)
%function [s, cx] = mcorrect1(s)
%
%Simple motion correction based on dftregrestration.m
%
%All images are aligned to the first image in the stack. Each
%image's shift and phase error relative to the first image is
%calculated and then they're shifted back to compensate. New
%corrected stack is returned.
%

cx = zeros([1 size(s.g, 3)-1]);
ref = fft2(s.g(:,:,1));
for n = 2:size(s.g, 3)
  im0 = s.g(:,:,n);
  im = fft2(im0);
  output = dftregistration(ref, im, 5);
  cx(n-1) = hypot(output(3), output(4));
  if output(3) > 0 || output(4) > 0
    s.g(:,:,n) = applyshift(s.g(:,:,n), ...
                            -output(2), -output(3), -output(4));
  end
end

  
function g = applyshift(f, phase, deltar, deltac)
[nr,nc]=size(f);
Nr = ifftshift([-fix(nr/2):ceil(nr/2)-1]);
Nc = ifftshift([-fix(nc/2):ceil(nc/2)-1]);
[Nc,Nr] = meshgrid(Nc,Nr);
g = abs(ifft2(fft2(f).*exp(i*2*pi*(deltar*Nr/nr+deltac*Nc/nc))).*exp(-i*phase));
...
% src: http://www.mathworks.com/matlabcentral/fileexchange/18401-efficient-subpixel-image-registration-by-cross-correlation/content/html/efficient_subpixel_registration.html
