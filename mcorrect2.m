function stack = mcorrect2(stack, referenceim)
% original author: patrick minault @ ucla (ringach lab)
% src: https://xcorr.net/2014/08/02/non-rigid-deformation-for-calcium-imaging-frame-alignment/

if ~exist('referenceim', 'var')
  T = mean(stack.g, 3);                   % align all frames to stack mean
else
  T = referenceim;
end
...
stack.dx = [];
stack.dy = [];
for n = 1:size(stack.g, 3)
  [stack.g(:,:,n) dx dy] = doLucasKanade(T, stack.g(:,:,n));
  stack.dx = [stack.dx dx];
  stack.dy = [stack.dy dy];
end
if nargout == 0
  subplot(2,2,2);
  ims(stack.g, 'corr');
